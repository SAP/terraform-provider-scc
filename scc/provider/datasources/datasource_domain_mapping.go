...
func (d *DomainMappingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data model.DomainMappingConfig
	...
	diags = helpers.RequestAndUnmarshal(d.Client, &respObj.DomainMappings, "GET", endpoint, nil, true)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	responseModel, diags := model.DomainMappingValueFrom(ctx, data, *mappingRespObj)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
