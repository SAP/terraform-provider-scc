...
func TestDataSourceDomainMapping(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		rec, user := tfutils.SetupVCR(t, "fixtures/datasource_domain_mapping")
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: tfutils.ProviderConfig(user) + DataSourceDomainMapping("scc_dm", regionHost, subaccount, internalDomain),
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("data.scc_domain_mapping.scc_dm", "region_host", regionHost),
						resource.TestMatchResourceAttr("data.scc_domain_mapping.scc_dm", "subaccount", tfutils.RegexpValidUUID),
