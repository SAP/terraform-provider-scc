...
func TestDataSourceDomainMappings(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		rec, user := tfutils.SetupVCR(t, "fixtures/datasource_domain_mappings")
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: tfutils.ProviderConfig(user) + DataSourceDomainMappings("scc_dms", regionHost, subaccount),
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("data.scc_domain_mappings.scc_dms", "region_host", regionHost),
						resource.TestMatchResourceAttr("data.scc_domain_mappings.scc_dms", "subaccount", tfutils.RegexpValidUUID),
