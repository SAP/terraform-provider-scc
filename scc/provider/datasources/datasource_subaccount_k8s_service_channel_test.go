...
func TestDataSourceSubaccountK8SServiceChannel(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		rec, user := tfutils.SetupVCR(t, "fixtures/datasource_subaccount_k8s_service_channel")
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: tfutils.ProviderConfig(user) + DataSourceSubaccountK8SServiceChannel("scc_k8s_sc", regionHost, subaccount, 2),
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("data.scc_subaccount_k8s_service_channel.scc_k8s_sc", "region_host", regionHost),
						resource.TestMatchResourceAttr("data.scc_subaccount_k8s_service_channel.scc_k8s_sc", "subaccount", tfutils.RegexpValidUUID),
