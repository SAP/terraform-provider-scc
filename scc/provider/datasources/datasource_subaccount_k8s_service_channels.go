...
func (d *SubaccountK8SServiceChannelsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data model.SubaccountK8SServiceChannelsConfig
	...
	diags = helpers.RequestAndUnmarshal(d.Client, &respObj.SubaccountK8SServiceChannels, "GET", endpoint, nil, true)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	responseModel, diags := model.SubaccountK8SServiceChannelsValueFrom(ctx, data, respObj)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
