...
func (d *SubaccountsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data model.SubaccountsConfig
	...
	diags = helpers.RequestAndUnmarshal(d.Client, &respObj.Subaccounts, "GET", endpoint, nil, true)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	responseModel, diags := model.SubaccountsDataSourceValueFrom(respObj)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
