...
func TestDataSourceSubaccounts(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		rec, user := tfutils.SetupVCR(t, "fixtures/datasource_subaccounts")
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: tfutils.ProviderConfig(user) + DataSourceSubaccounts("scc_sas"),
					Check: resource.ComposeAggregateTestCheckFunc(
						...
						resource.TestMatchResourceAttr("data.scc_subaccounts.scc_sas", "subaccounts.0.subaccount", tfutils.RegexpValidUUID),
