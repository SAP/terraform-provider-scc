...
func TestDataSourceSystemCertificate_Read_MetadataAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusInternalServerError)
	}))
	defer srv.Close()

	ds := &datasources.SystemCertificateDataSource{Client: tfutils.NewTestClient(t, srv)}
...
func TestDataSourceSystemCertificate_Read_BinaryAPIError(t *testing.T) {
	// Metadata succeeds but binary download fails.
	metadataResponse := map[string]any{
		"subjectDN":    "CN=test-system",
		"issuer":       "CN=test-system",
		"notAfter":     0,
		"notBefore":    0,
