...
func (d *SystemMappingsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data model.SystemMappingsConfig
	...
	diags = helpers.RequestAndUnmarshal(d.Client, &respObj.SystemMappings, "GET", endpoint, nil, true)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	responseModel, diags := model.SystemMappingsValueFrom(ctx, data, respObj)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
