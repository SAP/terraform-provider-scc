...
func TestDataSourceSystemMappings(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		rec, user := tfutils.SetupVCR(t, "fixtures/datasource_system_mappings")
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: tfutils.ProviderConfig(user) + DataSourceSystemMappings("scc_sms", regionHost, subaccount),
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("data.scc_system_mappings.scc_sms", "region_host", regionHost),
						resource.TestMatchResourceAttr("data.scc_system_mappings.scc_sms", "subaccount", tfutils.RegexpValidUUID),
						...
						resource.TestMatchResourceAttr("data.scc_system_mappings.scc_sms", "system_mappings.0.creation_date", tfutils.RegexpValidTimeStamp),
