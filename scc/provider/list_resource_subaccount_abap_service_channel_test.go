	...
	t.Run("happy path", func(t *testing.T) {
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Query: true,
					Config: providerConfig(user) + listSubaccountABAPServiceChannelQueryConfig("test", "scc",
						regionHost, subaccount),

					QueryResultChecks: []querycheck.QueryResultCheck{
						querycheck.ExpectLength("scc_subaccount_abap_service_channel.test", 1),
				...
				{
					Query: true,
					Config: providerConfig(user) + listSubaccountABAPServiceChannelQueryConfigWithIncludeResource("test", "scc",
						regionHost, subaccount),
