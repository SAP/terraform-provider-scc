...
func TestResourceDomainMapping(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		rec, user := tfutils.SetupVCR(t, "fixtures/resource_domain_mapping")
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: tfutils.ProviderConfig(user) + ResourceDomainMapping("scc_dm", regionHost, subaccount, virtualDomain, internalDomain),
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("scc_domain_mapping.scc_dm", "region_host", regionHost),
						resource.TestMatchResourceAttr("scc_domain_mapping.scc_dm", "subaccount", tfutils.RegexpValidUUID),
				...
				{
					Config:      tfutils.ProviderConfig(user) + ResourceDomainMapping("scc_dm", "cf.us10.hana.ondemand.com", subaccount, "updatedtfvirtualdomain", internalDomain),
					ExpectError: regexp.MustCompile(`(?is)error updating the cloud connector domain mapping.*mismatched\s+configuration values`),
				},
				{
					Config: tfutils.ProviderConfig(user) + ResourceDomainMapping("scc_dm", regionHost, subaccount, "updatedtfvirtualdomain", internalDomain),
					Check: resource.ComposeAggregateTestCheckFunc(
