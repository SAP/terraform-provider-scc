...
func TestResourceSubaccountK8SServiceChannel(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		rec, user := tfutils.SetupVCR(t, "fixtures/resource_subaccount_k8s_service_channel")
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: tfutils.ProviderConfig(user) + ResourceSubaccountK8SServiceChannel("scc_k8_sc", regionHost, subaccount, user.K8SCluster, user.K8SService, 3000, 1, false, "Created"),
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("scc_subaccount_k8s_service_channel.scc_k8_sc", "region_host", regionHost),
						resource.TestMatchResourceAttr("scc_subaccount_k8s_service_channel.scc_k8_sc", "subaccount", tfutils.RegexpValidUUID),
	...
	t.Run("update path - description and connections update", func(t *testing.T) {
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				{
					Config:      tfutils.ProviderConfig(user) + ResourceSubaccountK8SServiceChannel("scc_k8_sc", "cf.us10.hana.ondemand.com", subaccount, user.K8SCluster, user.K8SService, 3000, 1, false, "Updated"),
				...
				{
					Config: tfutils.ProviderConfig(user) + ResourceSubaccountK8SServiceChannel("scc_k8_sc", regionHost, subaccount, user.K8SCluster, user.K8SService, 3000, 2, false, "Updated"),
					Check: resource.ComposeAggregateTestCheckFunc(
