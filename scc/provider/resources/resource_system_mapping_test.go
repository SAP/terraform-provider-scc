...
func TestResourceSystemMapping(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		rec, user := tfutils.SetupVCR(t, "fixtures/resource_system_mapping")
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				// CREATE
				{
					Config: tfutils.ProviderConfig(user) + ResourceSystemMapping("scc_sm", regionHost, subaccount, virtualHost, virtualPort, internalHost, internalPort, "HTTP", "abapSys", "VIRTUAL", "KERBEROS"),
				...
				{
					Config:      tfutils.ProviderConfig(user) + ResourceSystemMapping("scc_sm", "cf.us10.hana.ondemand.com", subaccount, virtualHost, virtualPort, internalHost, internalPort, "HTTP", "abapSys", "VIRTUAL", "KERBEROS"),
					ExpectError: regexp.MustCompile(`(?is)error updating the cloud connector system mapping.*mismatched\s+configuration values`),
				},
				// UPDATE
				{
					Config: tfutils.ProviderConfig(user) + ResourceSystemMapping("scc_sm", regionHost, subaccount, virtualHost, virtualPort, "updatedlocal", "905", "HTTPS", "hana", "INTERNAL", "X509_GENERAL"),
					Check: resource.ComposeAggregateTestCheckFunc(
